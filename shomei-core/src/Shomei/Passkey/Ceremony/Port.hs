...
--
-- This port lets 'Shomei.Session.Authentication.Workflow' code orchestrate those ceremonies without
...
-- payloads, already a core dependency) plus the 'Shomei.Passkey.Domain' domain types.
...
-- ('Shomei.Test.InMemory.runWebAuthnCeremonyFake') drives tests without cryptography.
module Shomei.Passkey.Ceremony.Port
  ( WebAuthnCeremony (..),
    WebAuthnError (..),
    CredentialUserInfo (..),
    BeginCeremony (..),
