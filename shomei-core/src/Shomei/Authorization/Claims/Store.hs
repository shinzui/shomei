...
-- and the roles read from the 'Shomei.Authorization.Role.Store.RoleStore', returning a 'ClaimsDelta'
-- the core merges into the standard claims.
--
-- This is the same shape as the 'Shomei.Account.Notification.Store' port: Shōmei decides /when/, the
...
--
-- The delta's extra-claims object is filtered through 'Shomei.Authorization.Claims.Domain.mkExtraClaims'
...
-- @permissions@, @act@ — the full list is 'Shomei.Authorization.Claims.Domain.reservedClaimKeys'). Returning a
-- /delta/ rather than letting the hook rewrite the whole 'Shomei.Authorization.Claims.Domain.AuthClaims' keeps
...
module Shomei.Authorization.Claims.Store
  ( ClaimsEnricher (..),
    ClaimsDelta (..),
    emptyClaimsDelta,
    enrichClaims,
...
import Shomei.Authorization.Claims.Domain (Role, Scope, noExtraClaims)
import Shomei.Id (UserId)
import Shomei.Prelude

-- | What a host adds to a freshly minted token's claims. The roles here are merged with (not
-- substituted for) the roles the 'Shomei.Authorization.Role.Store.RoleStore' reports.
data ClaimsDelta = ClaimsDelta
  { extraRoles :: !(Set Role),
    extraScopes :: !(Set Scope),
    -- | reserved keys are dropped when this is merged; see 'Shomei.Authorization.Claims.Domain.mkExtraClaims'
    extraClaims :: !Object
  }
