...
-- exact tail that 'Shomei.Session.Authentication.Workflow.login' (non-MFA path), 'Shomei.Mfa.Workflow.completeMfa',
-- and 'Shomei.Mfa.Workflow.completePasswordlessLogin' share, factored out so the call sites
...
-- would otherwise form between 'Shomei.Session.Authentication.Workflow' (which calls 'issueSession') and
-- 'Shomei.Mfa.Workflow' (which also calls 'issueSession'). 'Shomei.Session.Authentication.Workflow' re-exports
-- 'issueSession' so the public interface remains @Shomei.Session.Authentication.Workflow.issueSession@.
module Shomei.Session.Workflow
  ( buildClaims,
    buildClaimsWith,
    buildEnrichedClaims,
    SessionOptions (..),
...
import Shomei.Authorization.Claims.Domain (AuthClaims (..), Scope, mkExtraClaims, noExtraClaims)
...
-- token pair so a caller (e.g. 'Shomei.Mfa.Workflow.completeMfa') can name the session in its
