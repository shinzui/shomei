...
module Shomei.Account.Admin.Workflow
  ( suspendUser,
    reinstateUser,
    deleteUser,
    revokeUserSessions,
...
import Shomei.Account.User.Store (UserStore, findUserById, updateUserStatus)
...
-- through 'Shomei.Session.Authentication.Workflow.verifyToken', and the next request is refused with
