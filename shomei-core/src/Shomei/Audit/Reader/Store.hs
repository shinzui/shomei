...
-- 'Shomei.Audit.Publisher.Store'. It exposes filtered, keyset-paginated reads over the
...
-- 'Shomei.Audit.Event.Domain.AuthEvent'; reconstruction is the caller's choice via
-- 'Shomei.Audit.Event.Codec.reconstructAuthEvent'. This keeps the storage read decoupled from
-- the JSON shape: an unrecognized future @event_type@ still lists (with its raw payload)
-- instead of breaking the whole query.
module Shomei.Audit.Reader.Store
  ( AuthEventReader (..),
    AuditEventQuery (..),
    AuditCursor (..),
    StoredAuthEvent (..),
