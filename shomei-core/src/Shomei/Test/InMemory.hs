...
-- 'Shomei.Account.Password.Hash.Store' fake tags and compares plaintext, the
-- 'Shomei.Session.Token.Generator' fake emits @rt-0@, @rt-1@, … and the
-- 'Shomei.SigningKey.Signer'/'Shomei.SigningKey.Verifier' fakes round-trip 'AuthClaims'
-- through JSON.
module Shomei.Test.InMemory
  ( World (..),
    ...
    --     'Shomei.SigningKey.Signer'/'Shomei.SigningKey.Verifier' interpreters — keeping
...
import Shomei.Authorization.Claims.Domain (AuthClaims, Issuer (..), Permission (..), Role (..))
import Shomei.Authorization.Claims.Store (ClaimsDelta, ClaimsEnricher, emptyClaimsDelta, runClaimsEnricherNull, runClaimsEnricherPure)
...
import Shomei.OAuth.AuthorizationCode.Domain
  ( AuthorizationCode (..),
...
data World = World
  { users :: !(Map UserId User),
    ...
    --     into 'definedRoles': the registry check lives in 'Shomei.Authorization.Role.Workflow.grantRoleTo',
    ...
    --     holds the /raw/ 'Shomei.Mfa.Totp.Algorithm.TotpSecret'; only the PostgreSQL boundary encrypts it.
...
-- | In-memory mirror of 'Shomei.Audit.Reader.Postgres.runAuthEventReaderPostgres' over the
-- 'World''s @publishedEvents@ log. Each event is projected with the shared
-- 'Shomei.Audit.Event.Codec.projectAuthEvent' (the same mapping the writer uses) and assigned a
