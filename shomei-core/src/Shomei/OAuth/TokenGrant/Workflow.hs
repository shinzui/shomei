...
-- that the session was minted by /this/ client — and then delegates to 'Shomei.Session.Authentication.Workflow.refresh',
...
module Shomei.OAuth.TokenGrant.Workflow
  ( TokenGrantError (..),
    grantErrorCode,
...
-- a wrong secret cost the same work — the same ordering 'Shomei.ServiceAccount.ClientCredentials.Workflow' uses.
authenticateClient ::
