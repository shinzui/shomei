...
-- | Pure tests for the EP-3 passkey enrollment workflows ('Shomei.Passkey.Workflow'),
-- driven over EP-2's in-memory stores and EP-1's deterministic fake 'WebAuthnCeremony'
-- interpreter via 'Shomei.Test.InMemory.runInMemory'. No HTTP, no cryptography.
--
...
module Shomei.Passkey.WorkflowSpec (tests) where
...
import Shomei.Account.User.Store (createUser)
...
import Shomei.Passkey.Workflow
