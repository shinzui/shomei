...
- [x] M2 (2026-07-10): Token-endpoint dispatcher arm (`tokenExchangeGrant`) for the exchange grant; RFC 8693 request parsing, optional client auth (`resolveExchangeClient`), `RemoteHost` on `/oauth/token` for client IP, `resource` rejected; in-process HTTP tests (both modes, `denyUnderImpersonation` inheritance, stale operator, gate/ceiling/gate-scope refusals, garbage subject). `cabal test shomei-servant` green (29 HTTP + 51 OpenAPI examples).
