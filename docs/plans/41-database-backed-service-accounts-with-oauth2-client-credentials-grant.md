...
- [x] M3 (2026-07-10): `base64` added to `shomei-servant.cabal` (`http-api-data` was already a dependency); `FormUrlEncoded` route compiles.
- [x] M3 (2026-07-10): `Shomei.Servant.OAuth` module: RFC 6749 §5.2 error rendering, client-auth extraction (basic + post), scope parsing, `TokenResponse`. Hand-written after surveying the Hackage index — see Decision Log.
- [x] M3 (2026-07-10): `POST /oauth/token` route (in `ShomeiRoutes`, unversioned) + the grant dispatcher `oauthTokenH`; 12 in-process HTTP assertions covering both client-auth methods and every error code, plus scope isolation. Servant suite 19 → 21.
- [x] M3 (2026-07-10): OpenAPI: route documented with an `OAuthError` schema and RFC 6749 error responses; `/oauth/token` excluded from the problem-details decoration; path count 33 → 34; spec regenerated byte-identically.
...
**2026-07-10 (M4) — capturing the process's stdout in a parallel test runner is a race, and it
produced a suite that passed and failed on alternate runs.** The first draft of the admin tests
recovered the once-printed secret with `hDuplicateTo` on the global `stdout`. Tasty runs cases in
parallel, so two capturing cases interleaved and read each other's output:

```text
run 1: All 22 tests passed (4.14s)
run 2: 2 out of 22 tests failed (61.11s)
```

The fix was a better design, not a lock: `Shomei.Admin.ServiceAccounts` now exposes
`createAction` / `rotateSecretAction` / `revokeAction` / `listAction`, which *return* what they
did, and `runServiceAccounts` is a thin printing wrapper over them. The tests call the actions and
assert on values. This also dropped a `temporary` dependency. **EP-7's recovery codes are printed
once too, and will face the same temptation.**

**2026-07-10 (M4) — `cabal test all` fails nondeterministically on this repo, and it is
ephemeral-PostgreSQL contention, not your code.** Each DB-backed tasty case spins up its own
PostgreSQL cluster via `Shomei.Migrations.TestSupport.withShomeiMigratedDatabase`. Run in
parallel, cluster startup blows a 60-second timeout:

```text
create delegated session persists actor:  FAIL
  Exception: Failed to start ephemeral PostgreSQL: TimeoutError
    (ConnectionTimeout {durationSeconds = 60, host = "…/pg--ff4244c06725c51b", port = 60707})
  error, called at test-support/Shomei/Migrations/TestSupport.hs:25
```

The failing *set differs between runs* (3 admin + 4 postgres, then 2 + 1), the failures are never
assertions, and every suite passes in isolation. `TASTY_NUM_THREADS=1 cabal test all` is green,
exit 0, all twelve suites — that is the command to use before believing a red `cabal test all`.
(Do **not** add `--test-options="--num-threads=1"`: `shomei-servant-openapi-test` is hspec and
dies with `unrecognized option '--num-threads=1'`, which then looks like a real failure.) This is
the same class as the MasterPlan's pre-existing `SupervisorSpec` note, and EP-4 adds four more
ephemeral-database cases, so it is now easier to trip.

**2026-07-10 (M4) — the plan's own Validation transcript does not run.** `cabal run shomei-server`
is ambiguous — the package and the executable share the name — and aborts with
`Error: [Cabal-7070] The run command is for running a single executable at once.` The working
form is `cabal run shomei-server:exe:shomei-server`. Separately, port 8080 may be occupied
(here, by an unrelated `ssh -L` tunnel), and `curl -sf http://localhost:8080/health` then
*succeeds* against the other process — a false green. Set `SHOMEI_PORT` and assert on the body
(`{"status":"ok"}`), not on curl's exit code. The Validation section below is corrected.

**2026-07-10 (M4) — the `invalid_scope` description lied about the empty-scope case.** The live
transcript showed `scope=` answering
`{"error":"invalid_scope","error_description":"requested scope exceeds the client's allowed scopes"}`,
which is false: nothing was requested. The workflow raises one `OAuthScopeInvalid` for both
refusals, so the handler now sends one description honest about both. Caught only by driving the
real endpoint — every unit and HTTP test asserted the *code*, never the prose.

**2026-07-10 (M3) — `baselineSpecs` would have documented a problem-details 400 on `/oauth/token`,
silently contradicting the plan's whole premise.** `Shomei.Servant.OpenApi.baselineSpecs` adds
`pcBodyParseError` to any operation carrying a request body. `/oauth/token` carries one, so the
generated document would have promised `application/problem+json` on the very endpoint whose
reason for existing is that it answers RFC 6749 error objects — and no existing conformance check
would have noticed, because they only ever inspect `problem+json` responses. `oauthPaths` now
excludes it and `decoratePath` attaches an `OAuthError`-schema response per status instead. Two
new conformance assertions pin this: "documents no problem+json response on /oauth/token" and
"documents the RFC 6749 error codes it can actually emit". **EP-5 adds `/.well-known/openid-configuration`,
introspection, revocation and userinfo — introspection and revocation are `/oauth/*` endpoints and
must join `oauthPaths`.**

**2026-07-10 (M3) — the servant HTTP suite is a FIFTH effect-stack copy.** After the four M1 found
(`Seam`, `Server.App`, `InMemory`, `shomei-postgres/test`), `shomei-servant/test/Main.hs` has its
own `runHybrid` — in-memory stores plus the real `jose` signer. A port missing there fails only
when that suite compiles.

**2026-07-10 (M3) — a `testCase` that is never added to the tasty list is a silent no-op, and it
bit twice in this plan.** The M2 note below describes it for `shomei-core`. It recurred here: an
`Edit` that failed with "file has been modified since read" was not retried, so
`scenarioOAuthToken` compiled, was referenced by its `where`-bound env builder, and never ran —
while `cabal test` cheerfully reported `All 19 tests passed`. Only re-grepping the output for the
test's own name caught it:

```text
$ cabal test shomei-servant:shomei-servant-test | grep -i 'oauth/token'
(no output)
```

**Verify a new test by name in the runner's output, not by the suite going green.**
...
- Decision (M4): The CLI generates its secret with `Shomei.Crypto.generateOpaqueToken` rather
  than adding `crypton` to the `shomei-admin` executable for `getRandomBytes`.
  Rationale: `generateOpaqueToken` already *is* "32 bytes from the CSPRNG, rendered
  base64url-unpadded" — the exact primitive the plan called for — and it is already exported and
  already used to mint refresh tokens. Reusing it adds no dependency, and keeps one definition of
  "a secure opaque secret" in the codebase.
  Date: 2026-07-10

- Decision (M4): `Shomei.Admin.ServiceAccounts` exposes `createAction`/`rotateSecretAction`/
  `revokeAction`/`listAction` returning values, with `runServiceAccounts` a printing wrapper.
  Rationale: The generated secret is printed once and never stored, so a test must observe it to
  authenticate with it. The obvious route — capture the process's stdout — is a data race in a
  runner that executes cases in parallel, and it produced a suite that alternated between
  "All 22 tests passed" and "2 out of 22 tests failed" (see Surprises). Returning the value is
  simpler than a lock, removes a `temporary` dependency, and gives the tests something to assert
  on besides formatted prose.
  Date: 2026-07-10

- Decision (M4): `revoke` does not revoke the account's outstanding sessions or tokens.
  Rationale: Access tokens are stateless JWTs verified against the JWKS, so revoking a session row
  would not invalidate a token already in a client's hands; it would only create the illusion of
  having done so. The honest bound is the TTL (default five minutes), which the CLI now prints as
  a note on revoke. Deployments needing instant kill already have the answer: shorten the TTL, or
  put the fine-grained, live-revocable layer (en) in front of the resource. Recorded rather than
  silently omitted, because "revoke" is a word operators trust.
  Date: 2026-07-10

- Decision (M3): `POST /oauth/token` is a field of `ShomeiRoutes`, not `ShomeiAPI`.
  Rationale: The plan predates plan 40 (EP-3), which landed the `/v1` split. `ShomeiAPI` is the
  application record and is mounted under `/v1`; `ShomeiRoutes` holds the unversioned protocol and
  infrastructure endpoints. Putting the route in `ShomeiAPI` would serve it at
  `/v1/oauth/token`, contradicting this plan's own unversioned-path decision. A consequence worth
  knowing: `ShomeiRoutes` is what `shomei-client` derives `genericClient` from, so the client
  picked the route up with no change.
  Date: 2026-07-10

- Decision (M3): `POST /oauth/token` is NOT added to the rate limiter's `throttledPath` list.
  Rationale: The limiter is per-IP, and it exists to slow online guessing of *human-chosen*
  passwords on `/v1/auth/login` and friends. A service-account secret is 32 bytes of server-
  generated randomness, so online guessing is not a threat model. Meanwhile a fleet of services
  behind one NAT egress IP shares a single bucket, and every one of them re-fetches a token every
  five minutes (the default TTL) — throttling would produce correlated, self-inflicted outages
  precisely under load. If a deployment wants a bound here, it belongs at the ingress, keyed on
  `client_id` rather than IP. Revisit if EP-5 adds a browser-facing grant to this endpoint, where
  the credential *is* human-chosen. `testThrottledPathsAreVersioned` therefore needs no change.
  Date: 2026-07-10

- Decision (M3): Write `Shomei.Servant.OAuth` by hand rather than adopt an existing OAuth2
  server library. Surveyed 2026-07-10 against the Hackage index available to this build.
  Rationale: The only server-side candidate is **`oauth2-server` 0.3.0.0** (DPella, MPL-2.0), and
  four independent facts disqualify it:
  (a) **it does not implement `client_credentials`** — `Web/OAuth2/MetadataAPI.hs:100` declares
  `grant_types_supported = ["authorization_code", "refresh_token"]` and its `TokenRequest`
  (`Web/OAuth2/TokenAPI.hs:66`) carries only `code` and `refresh_token`, so it delivers none of
  this plan;
  (b) its state is an `MVar (OAuthState usr)` passed explicitly into every handler
  (`handleMetadata :: MVar (OAuthState usr) -> Handler OAuthMetadata`) with no persistence
  abstraction, while Shōmei's service accounts must live in PostgreSQL behind the
  `ServiceAccountStore` port and be revocable at runtime;
  (c) it mints tokens through `Servant.Auth.Server`, whereas Shōmei signs with `jose` in
  `Shomei.Jwt.Sign` under staged key rotation — a token signed by a second, unrelated JWT stack
  would not verify against the published `/.well-known/jwks.json`, which is this plan's central
  acceptance criterion;
  (d) it depends on `cryptonite` (deprecated in favor of `crypton`, which Shōmei already uses)
  and on `blaze-html`/`servant-blaze` for a built-in HTML login form — a hosted login UI is
  permanently out of scope per the MasterPlan Decision Log.
  `openid-connect` 0.2.0 was also evaluated: its entire provider-side surface is
  `OpenID.Connect.Provider.Key` (key generation); `OpenID.Connect.Discovery` is not even an
  exposed module. Every other match on the index (`hoauth2`, `servant-oauth2`,
  `oauth2-jwt-bearer`, `oauthenticated`, `authenticate-oauth`) is an OAuth2 *client* or OAuth 1.0a.
  What remains to hand-write is roughly 150 lines of wire mechanics — the RFC 6749 §5.2 error
  object, the Basic-header split, and form-parameter lookups — with no algorithmic content, and it
  plugs directly into the existing signer and ports.
  **EP-5 should not repeat this survey blindly**: `oauth2-server` does implement
  `authorization_code` with PKCE and a discovery document, so it is worth a second look there —
  but (b), (c) and (d) apply unchanged, so adopting it would still mean replacing its state,
  its signer, and its UI.
  Date: 2026-07-10
...
# NB: `cabal run shomei-server` is AMBIGUOUS (the package and the executable share the name) and
# aborts with Cabal-7070. Name the component. Port 8080 is often taken; if something else is
# listening there, `curl -sf .../health` succeeds against IT and you get a false green — so pick
# a port and check the BODY.
DATABASE_URL="$PG_CONNECTION_STRING" SHOMEI_PORT=8099 cabal run shomei-server:exe:shomei-server &
until curl -s http://127.0.0.1:8099/health | grep -q '"status"'; do sleep 1; done

curl -si -u "svcacct_01...:THE_SECRET" \
  -d 'grant_type=client_credentials&scope=kawa:ingest' \
  http://127.0.0.1:8099/oauth/token
```
