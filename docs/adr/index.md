...
- [Embedded hosts install the complete runtime boundary](0017-embedded-hosts-install-the-complete-runtime-boundary.md) - Hosts embedding Shomei install its background services and wrap the whole application in the standalone edge middleware.
- [Downstream JWKS refresh is bounded by local policy](0018-downstream-jwks-refresh-is-bounded-by-local-policy.md) - Resource services fetch keys over TLS, cap publisher freshness, and rate-limit one retry for an unknown key identifier.
